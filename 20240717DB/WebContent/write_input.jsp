...
String sqlInsert="insert into tblboard(numseq, name,pass,email,title,contents,writedate,readcount,num) values(num_seq.nextval,?,?,?,?,?,?,?,?)";
Class.forName(driver);    //1)외부파일을 가져오는(로딩) 방법
Connection conn=DriverManager.getConnection(url, user, password); //2)DB연동
// endDB관련 //

PreparedStatement pstmt=null;   //3) 준비된통 preparedstatement
...
pstmt=conn.prepareStatement(sqlInsert);   //3) 준비된통 preparedstatement
